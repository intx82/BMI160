...
class BMM150:
    ...
    XY_SCALE = const(630.15)  #  (2 ** 13) / 13G
    Z_SCALE = const(655.36)  # (2 ** 14) / 25G
    ...
    def __init__(self, bmi: BMI160_I2C) -> None:
        ...
        self._reg_write_bits(registers.MAG_IF_1, 0, 7, 1)
        # Wait for power-up to complete
        while (0x1 != self._reg_read_bits(registers.PMU_STATUS, 0, 2)):
            sleep_ms(1)

    def _reg_write(self, reg, data):
        self._bmi._reg_write(reg, data)
    ...
    def _reg_read(self, reg):
        return self._bmi._reg_read(reg)
    ...
    def read_raw(self):
        ...
        #return unpack('<hhh', bytes([regs[0] >> 3, regs[1], regs[2] >> 3, regs[3], regs[4] >> 2, regs[5]]))
